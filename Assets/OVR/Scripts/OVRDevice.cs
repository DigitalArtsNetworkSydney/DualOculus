...
public class OVRDevice : MonoBehaviour 
{
	...
	{	
		...
		// ---- TBWA ----
		Debug.Log("SensorCount: " + SensorCount + " | "
		          + "SensorList.Count: " + SensorList.Count);
		// ---- TBWA ----

		// PredictionTime set, to init sensor directly
		if(PredictionTime > 0.0f)
		{
            OVR_SetSensorPredictionTime(SensorList[0], PredictionTime);   // TBWA
			OVR_SetSensorPredictionTime(SensorList[1], PredictionTime);   // TBWA
		}
		else
		{
			SetPredictionTime(SensorList[0], InitialPredictionTime);   // TBWA
			SetPredictionTime(SensorList[1], InitialPredictionTime);   // TBWA
		}
	}
