...
{	
	...
	private OVRCamera CameraRight = null;
	...
	void SetCameraOrientation()
	{
			...
			{		
				if(CameraController.EnableOrientation == true)
				{
					...
					if(CameraController.PredictionOn == false)
						OVRDevice.GetOrientation(SensorIndex, ref CameraOrientation);   // TBWA
					else
					{
						OVRDevice.GetPredictedOrientation(SensorIndex, ref CameraOrientation);   // TBWA
					}
				}
			}
		...
		{
			// Left camera
			CameraOrientation = CameraRight.CameraOrientation;
		}
